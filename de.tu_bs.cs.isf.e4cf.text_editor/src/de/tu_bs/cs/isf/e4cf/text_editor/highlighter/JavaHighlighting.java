...
public class JavaHighlighting {
    ...
    public static StyleSpans<Collection<String>> computeHighlighting(String text) {
		...
		while(matcher.find()) {
			String styleClass = 
					...
					matcher.group("PARAMETER") != null ? "java-param" :
					null;	
			Highlight highlight = new Highlight(styleClass, matcher.start(), matcher.end());
            highlights.add(highlight);
		}
		// sort the list based on startIndex, highlight will then store the data sequentially
		highlights = highlights
                .stream()
                .sorted(new Comparator<Highlight>(){
                    @Override
                    public int compare(Highlight s1, Highlight s2){
                        return Integer.compare(s1.getStartIndex(), s2.getStartIndex());
                    }
                }).collect(Collectors.toList());
        for(int i = 0; i < highlights.size(); i++){
            Highlight firstHighlight = highlights.get(i);
            spansBuilder.add(Collections.singleton(firstHighlight.getCss()), firstHighlight.getLength());

            if(i < highlights.size() - 1){
                Highlight secondHighlight = highlights.get(i + 1);
                spansBuilder.add(Collections.emptyList(), secondHighlight.getStartIndex() - firstHighlight.getEndIndex());
            }
        }
    ...
    /**
     * Adding a method to check if the user want to call instance
     * If the instance has not been initialized then it will mark it with java-error css
     * otherwise we highlight the name of instance
     * 
     * @param text content on codeArea that will be checked
     * @return adding it to List of highlight
     * @author Erwin Wijaya
     */
	 private static List<Highlight> errorInstanceHighlighting(String text){
	        List<Highlight> highlights = new ArrayList<>();
	  ...
	  * if the class name is not the same then it will be highlighted
	  * otherwise it will return as no error
	  * 
	  * @param text content on codeArea that will be checked
	  * @return adding it to List of highlight
	  * @author Erwin Wijaya
	  */
	 private static List<Highlight> errorClassHighlighting(String text){
	        List<Highlight> highlights = new ArrayList<>();
	        ...
	        while(matcher.find()){
	            ...
	            if(secondClassInLineMatcher.find()){
	                ...
	                if(!firstClassName.equals(secondClassName)){
	                    highlights.add(new Highlight("java-error", firstClassStartIndex, firstClassEndIndex));
	                    highlights.add(new Highlight("java-error", secondClassStartIndex, secondClassEndIndex));
	                }
	            }
	        }
	        return highlights;
