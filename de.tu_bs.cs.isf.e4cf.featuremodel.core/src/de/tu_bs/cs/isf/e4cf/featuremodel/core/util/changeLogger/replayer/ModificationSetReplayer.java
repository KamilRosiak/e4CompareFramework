...
public class ModificationSetReplayer {
	...
	public ModificationSetReplayer(FeatureModelModificationSet modificationSet, List<FXGraphicalFeature> features,
	...
	public void replay(FeatureDiagramm currentModel) {
		for (Modification modification : _modificationSet.getModifications()) {
			if (dispatchModification(modification)) {
				...
				throw new NullPointerException("Feature with id=" + modification.getFeatureID()
						+ " is not recognized by the modification replayer.");
			}
		}
	}
	...
	private boolean replayFeatureSelectionGroupInFeautre(Modification modification) {
		_services.eventBroker.send(FDEventTable.GROUP_SELECTED_FEATURES_IN_FEATURE,
				_featureIDs.get(modification.getFeatureID()));
		return true;
	}

	private boolean replayFeatureSelectionMoveUnderFeautre(Modification modification) {
		_services.eventBroker.send(FDEventTable.MOVE_SELECTED_FEATURES_UNDER_FEATURE,
				_featureIDs.get(modification.getFeatureID()));
		return true;
	}
	...
	private boolean replayFeatureGroupVariability(Modification modification) {
		...
		if (valueAfterChange.equals("alternative")) {
			_services.eventBroker.send(FDEventTable.SET_FEATURE_ALTERNATIVE_GROUP_BY_LOGGER,
					_featureIDs.get(modification.getFeatureID()));
		} else if (valueAfterChange.equals("and")) {
			_services.eventBroker.send(FDEventTable.SET_FEATURE_AND_GROUP_BY_LOGGER,
					_featureIDs.get(modification.getFeatureID()));
		} else if (valueAfterChange.equals("or")) {
			_services.eventBroker.send(FDEventTable.SET_FEATURE_OR_GROUP_BY_LOGGER,
					_featureIDs.get(modification.getFeatureID()));
		}
	...
	private boolean replayFeatureVariability(Modification modification) {
		if (modification.getDelta().getValuePriorChange().equals("mandatory")) {
			...
			_services.eventBroker.send(FDEventTable.SET_FEATURE_MANDATORY,
					_featureIDs.get(modification.getFeatureID()));
		}
		return true;
	}
	...
	private boolean replaySubfeatureVisibility(Modification modification) {
		_services.eventBroker.send(FDEventTable.CHANGE_SUBFEATURES_VISIBILITY,
				_featureIDs.get(modification.getFeatureID()));
		return true;
	}

	/**
	 * Changes the Abstraction of a feature. Internals are done in the
	 * FeatureModelEditorView, including assessing the current abstraction of a
	 * feature and then switching it. Hence, there is not separate method of making
	 * a feature concrete
	 * 
	 * @param modification
	 * @return
