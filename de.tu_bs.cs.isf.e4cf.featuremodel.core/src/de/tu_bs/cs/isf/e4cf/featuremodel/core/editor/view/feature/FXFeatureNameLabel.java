...
public class FXFeatureNameLabel extends Label {
	...
	public void showRenameFeatureDialog() {
		...
		FMESimpleTextInputDialog dialog = new FMESimpleTextInputDialog(FDStringTable.FD_DIALOG_MENU_RENAME_FEATURE,
				fxGraFeature.getFeature().getName());
		String newName = dialog.show(Double.valueOf(Display.getCurrent().getCursorLocation().x),
				Double.valueOf(Display.getCurrent().getCursorLocation().y));
