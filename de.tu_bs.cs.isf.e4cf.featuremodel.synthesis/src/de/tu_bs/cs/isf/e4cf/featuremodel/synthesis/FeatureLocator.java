...
public class FeatureLocator {
	...
	public FeatureDiagram updateMPL(MPLPlatform currentMpl, MPLPlatform newMpl) {
		...
		List<IFeature> newFeatures = groups.stream().map(Cluster::new).map(FeatureUtil::toFeature).collect(Collectors.toList());
		return updateFeatureModel(currentMpl.getFeatureModel().get(), newFeatures, newConfig);
	}
	
	private FeatureDiagram updateFeatureModel(FeatureDiagram oldDiagram, List<IFeature> newFeatures, Configuration newConfig) {
		...
		while (!featureMatchStack.isEmpty()) {
			...
			//newSubsetFeatures.forEach(f -> System.out.println(f.getArtifactUUIDs().size()));
		...
		// create hierarchy by inserting matched children of oldFeatures in new diagram using feature map
	...
	private List<IFeature> getDependencyOverlaps(IFeature targetFeature, List<IFeature> features) {
		return checkFeatureCondition(features, f -> !Collections.disjoint(targetFeature.getArtifactUUIDs(), f.getArtifactUUIDs()));
	}
	...
	private void updateFeatures(Collection<IFeature> oldFeatures, Collection<IFeature> newFeatures, Configuration newConfig) {
		String newVariantName = newConfig.getName();
