...
public class ArchVisitor extends ArchParserGrammarBaseVisitor<Node> {
	...
	@Override public Node visitAttribute(@NotNull ArchParserGrammar.AttributeContext ctx) {
		Node attr = new NodeImpl(NodeType.ARCH_ATTRIBUTE);
	...
	public Node visitContent(@NotNull ArchParserGrammar.ContentContext ctx) {
		Node temp = new NodeImpl(NodeType.ARCH_CONTENT);
	...
	private NodeType getElementType(String elementName) {
		switch (elementName) {
			case "signals"		: return NodeType.ARCH_SIGNAL;
			case "ports"		: return NodeType.ARCH_PORT;
			case "connectors"	: return NodeType.ARCH_CONNECTOR;
			case "ownedComments": return NodeType.ARCH_OWNED_COMMENT;
	...
	private void handleParsedNode(Pair<Node, Node> parentParsedPair) {
		...
		if (parsed.getStandardizedNodeType().equals(NodeType.ARCH_ATTRIBUTE)) {
			parent.addAttribute(parsed.getAttributes().get(0));
		} else if (parsed.getStandardizedNodeType().equals(NodeType.ARCH_CONTENT)) {
