...
public class DimacsCnfChecker implements FeatureConfigurationChecker {
	...
	protected Map<IFeature, Integer> featureToVariable;
	...
	public void initialize(FeatureConfiguration fc) {
		config = fc;
		if (config == null || config.getDiagram() == null) {
			isInit = false;
			return;
		}
		FeatureDiagram fd = config.getDiagram();

		// assign each feature to an integer
		featureToVariable.clear();
		for (int i = 0; i < config.getSelectedFeatures().size(); i++) {
			featureToVariable.put(config.getSelectedFeatures().get(i), i + 1);
		}

		// formulate a satisfiability problem in CNF in the DIMACS format
		for (Iterator<IFeature> iterator = new FeatureDiagramIterator(fd); iterator.hasNext();) {
	...
	private void printProblem() {
		System.out.println("Checking Feature Selection for " + config.getDiagram().getRoot().getName());
		System.out.println("Feature Mapping: ");
		featureToVariable
				.forEach((feature, variable) -> System.out.println("\t > " + feature.getName() + " -> " + variable));

		System.out.println("Clauses: ");
		for (List<Integer> clause : dimacsCnfBuilder.clauses) {
			String clauseString = String.join(", ",
					clause.stream().map(variable -> variable.toString()).collect(Collectors.toList()));
	...
	/**
	 * Derives a CNF from a feature. It adds the clauses directly to the DIMACS
	 * builder instance.
	 * 
	 * @param feature
	 */
	private void deriveClauses(IFeature feature) {
		...
		if (!featureSelected) {
			...
			// add feature and parent feature facts if the feature is selected but the
			// parent feature is not, thus invalidating the clause
			IFeature parentFeature = feature.getParent().get();
			...
			if (isSelected(feature) && feature.getVariability().equals(Variability.MANDATORY)) {
		...
		for (IFeature childFeature : feature.getChildren()) {
			if (childFeature.getVariability().equals(Variability.MANDATORY)) {
		...
		IFeature parentFeature = feature.getParent().get();
		if (parentFeature != null) {
			if (featureSelected && feature.getGroupVariability().equals(GroupVariability.ALTERNATIVE)) {
			...
			if (featureSelected && parentFeature.getGroupVariability().equals(GroupVariability.OR)) {
			...
			if (featureSelected && !feature.getGroupVariability().equals(GroupVariability.OR)
					&& !feature.getGroupVariability().equals(GroupVariability.ALTERNATIVE)) {
				for (IFeature childFeature : feature.getChildren()) {
	...
	private List<List<Integer>> createAlternativeClauses(List<IFeature> features) {
		...
		// add clause that enforces that no pair of features are selected at the same
		// time
		for (int i = 0; i < childVariables.length; i++) {
	...
	private boolean isSelected(IFeature feature) {
		if (config != null) {
			Boolean featureSelected = config.isSelected(feature);
