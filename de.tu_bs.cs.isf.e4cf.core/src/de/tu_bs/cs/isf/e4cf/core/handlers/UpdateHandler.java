...
public class UpdateHandler {
	 private static final String REPOSITORY_LOC = System.getProperty("UpdateHandler.Repo", "http://localhost/repository");
	    ...
	    @Execute
	    public void execute(final IProvisioningAgent agent, IWorkbench workbench, ServiceContainer services) {
	        this.workbench = workbench;
	        Job updateJob = Job.create("Update Job", monitor -> {
	            performUpdates(agent, monitor);
	        });
	        updateJob.schedule();
	        RCPMessageProvider.informationMessage("E4CompareFramework", "will be available soon ");
	    }
