...
bool check2(int x1,int y1,int x2,int y2,int x,int y)
{
	return x<x1 || x>x2 || y<y1 || y>y2;
}
bool check1(int x1,int y1,int x2,int y2,int x,int y)
{
	checkmin(x1,x);
	checkmax(x2,x);
	checkmin(y1,y);
	checkmax(y2,y);
	for (int i=0;i<SIZE(B);i++) if (!check2(x1,y1,x2,y2,B[i].first,B[i].second)) return false;
	return true;
}
int main()
{
...
//	freopen("..\\A-small-attempt1.in","r",stdin);freopen("..\\A-small-attempt1.out","w",stdout);
	...
	for (int caseId=1;caseId<=testcase;caseId++)
	{
		...
		for (int i=0;i<n;i++)
		{
			...
			scanf("%d%d",&x,&y);
			scanf("%s",str);
			if (str[0]=='B') A.push_back(MP(x,y));
			else { scanf("%s",str); B.push_back(MP(x,y)); }
		}
		scanf("%d",&m);
		if (SIZE(A)==0)
		{
			for (;m>0;m--)
			{
				...
				ipair data=MP(x,y);
				bool fd=false;
				for (int i=0;i<SIZE(B);i++) if (B[i]==data) fd=true;
				if (fd) printf("NOT BIRD\n");
				else printf("UNKNOWN\n");
			}
		}
		else
		{
			...
			{
				checkmin(x1,A[i].first);
				checkmax(x2,A[i].first);
				checkmin(y1,A[i].second);
				checkmax(y2,A[i].second);
			}
			for (;m>0;m--)
			{
				int x,y;
				scanf("%d%d",&x,&y);
				bool b1=check1(x1,y1,x2,y2,x,y);
				bool b2=check2(x1,y1,x2,y2,x,y);
				if (b1 && !b2) printf("BIRD\n");
				else if (!b1 && b2) printf("NOT BIRD\n");
				else printf("UNKNOWN\n");
			}
		}
		fflush(stdout);
