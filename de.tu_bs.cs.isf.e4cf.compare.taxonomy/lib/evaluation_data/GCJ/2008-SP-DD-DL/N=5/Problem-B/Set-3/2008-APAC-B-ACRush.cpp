...
{
	if (A[kX][kY]==0) return;
	if (A[kX-1][kY]+A[kX+1][kY]+A[kX][kY-1]+A[kX][kY+1]==0) result=1000000000;
	...
	for (int step=0;step<2;step++)
		...
		{
			int estep=1;
			if (d>=0 && A[kX+MX[d]][kY+MY[d]]>0) estep=0;
			if (estep!=step) continue;
			int T[maxsize][maxsize],D[maxsize][maxsize];
			memcpy(T,A,sizeof(A));
			for (int i=1;i<=n;i++) for (int j=1;j<=m;j++) if (A[i][j]>0)
			{
				int k=0;
				for (int d=0;d<4;d++) if (A[i+MX[d]][j+MY[d]]>A[i+MX[k]][j+MY[k]]) k=d;
				if (A[i+MX[k]][j+MY[k]]==0) k=-1;
				D[i][j]=k;
			}
			D[kX][kY]=d;
			for (int i=1;i<=n;i++) for (int j=1;j<=m;j++) if (D[i][j]>=0)
			{
				int x=i+MX[D[i][j]];
				int y=j+MY[D[i][j]];
				A[x][y]=max(0,A[x][y]-T[i][j]);
			}
			if (A[kX][kY]>0)
				search2(depth+1);
			memcpy(A,T,sizeof(A));
...
void solve()
{
	...
	for (int L=0;1;L++)
	{
		maxdepth=L;
		search2(0);
		if (result<L || result==1000000000) return;
	}
}
int main()
{
...
//	freopen("..\\B-small-attempt2.in","r",stdin);freopen("..\\B-small-attempt2.ans","w",stdout);
